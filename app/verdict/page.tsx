        ...
        <div className="max-w-xl mx-auto">
          ...
          {done ? (
            <div className="card-beef py-12 text-center">
              ...
              <p className="text-beef-text-muted text-sm mb-8">Your verdict is in. We&apos;ll take it seriously.</p>
              <div className="flex justify-center gap-4">
                <button onClick={() => { setDone(false); setContent(""); setEmail(""); setCategory("general"); }}
                  className="text-xs font-bold tracking-widest text-beef-gold hover:text-beef-gold/80 border border-beef-gold/40 px-4 py-2 transition-colors">SUBMIT ANOTHER</button>
                <Link href="/"><button className="btn-primary text-xs px-4 py-2">BACK TO HOME</button></Link>
              </div>
            </div>
          ) : (
            <div className="card-beef">
                ...
                {CATEGORIES.map((c) => (
                  <button key={c.value} onClick={() => setCategory(c.value)}
                    className={`text-left px-3 py-3 border transition-colors ${
                      category === c.value ? "border-beef-gold bg-beef-gold/5 text-beef-gold" : "border-beef-border text-beef-text-muted hover:border-beef-gold/40"
                    }`}>
              ...
              <textarea value={content} onChange={(e) => setContent(e.target.value.slice(0, 2000))}
                placeholder="Say what needs saying..." rows={6}
